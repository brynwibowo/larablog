...
class Posts extends Component
{
    ...
    public function mount($slug){
        $post = Content::leftJoin('users','users.id','=','contents.author')
                ...
                ->whereNotIn('contents.kategori',['#Gallery','#Profile','#Santri',])
                ->where('contents.is_published','=',true)->first();
        if ($post)
        {
    ...
    {
        $contentx = Content::leftJoin('users','users.id','=','contents.author')
        ...
        $santrix = Content::leftJoin('users','users.id','=','contents.author')
                    ->select('contents.judul','contents.photo','contents.slug','users.name','contents.published_at')
