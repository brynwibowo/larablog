...
class Contents extends Component
{
    ...
    public $urutan = "DESC";
    public $protectKategori = ['#Gallery','#Profile'];
    ...
    public function create(){
     ...
     public function publikasi($ids)
     {
      if(Auth::user()->level == 'admin')
        ...
        $this->alertSuccess("Konten berhasil dipublikasi!"); //menampilkan notifikasi
       }
     }
    ...
    public function edit($ids){
        ...
        $this->kategori = $content->kategori;
    ...
    public function delete($id){
        ...
        if($id){
            ...
            $this->emit('contentDeleted'); //menutup modal delete
      ...
      if(Auth::user()->level == 'admin')
       {  
        ...
        $contents = Content::leftJoin('users','users.id','=','contents.author')
                    ->select('contents.id','contents.judul','contents.kategori','contents.slug','users.name','contents.views','contents.is_published','contents.published_at')
                    ->whereNotIn('contents.kategori',$this->protectKategori)
                    ->orderBy('contents.id',$this->urutan)->paginate(10);

        if ($this->search != '') { //jika kolom search/cari diisi, memanggil data berdasarkan kata kunci dan kategori
            $contents = Content::leftJoin('users','users.id','=','contents.author')
                    ->select('contents.id','contents.judul','contents.kategori','contents.slug','users.name','contents.views','contents.is_published','contents.published_at')
                    ->whereNotIn('contents.kategori',$this->protectKategori)
                    ->where('contents.'.$this->kategoriOps, 'like', '%' . $this->search . '%')
                    ->orderBy('contents.id',$this->urutan) //urutan bernilai ASC akan memanggil data dari yg terlama, DESC terbaru
                    ->paginate(10); //paginasi tiap 10 data
        }
        return view('admin.contents',['contents'=>$contents]) //memanggil view untuk ditampilkan
           ->layout('layouts.alela'); //layout view yang digunakan
        }else{
            //akan dieksekusi jika level pengguna penulis
            //memanggil data untuk ditampilkan di page konten hanya jika sesuai dengan penulisnya
            
            $contents = Content::join('users','users.id','=','contents.author')
                    ->select('contents.id','contents.judul','contents.slug','contents.views','contents.is_published','contents.published_at','users.name')
                    ->where('contents.kategori','#Santri')
                    ->where('contents.author',Auth::user()->id)
                    ->orderBy('contents.id',$this->urutan)->paginate(10);
                ...
                if ($this->search != '') { //jika kolom search/cari diisi, memanggil data berdasarkan kata kunci dan kategori
                    $contents = Content::join('users','users.id','=','contents.author')
                                ...
                                ->where('contents.author',Auth::user()->id)
                                ->where('contents.'.$this->kategoriOps, 'like', '%' . $this->search . '%')
                                ->orderBy('contents.id',$this->urutan)
                                ->paginate(10); //paginasi tiap 10 data
                    }
                        return view('writer.contents',['contents'=>$contents]) //memanggil view untuk ditampilkan
                        ->layout('layouts.alela'); //layout view yang digunakan
